...
namespace vlko.core.Models.Action.Implementation
{
    public class UserAuthentication : IUserAuthentication
    {
        ...
        public CreateUserStatus CreateUser(string username, string password, string email, out string verifyToken)
        {
            ...
            if (ActiveRecordLinq.AsQueryable<User>().FirstOrDefault(user => user.Name == username) != null)
            {
                return CreateUserStatus.DuplicateUserName;
            }

            // check for unique email
            if (ActiveRecordLinq.AsQueryable<User>().FirstOrDefault(user => user.Email == email) != null)
            {
