    ...
    public static partial class SqlMapperExtensions
    {
        ...
        public static async Task<T> GetAsync<T>(this IDbConnection connection, dynamic id, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            if (!GetQueries.TryGetValue(type.TypeHandle, out string sql))
            {
                ...
                sql = $"SELECT * FROM {name} WHERE {GetFormatter(connection).FormatColumnName(key.Name)} = @id";
        ...
        /// <param name="commandTimeout">Number of seconds before command execution timeout</param>
        /// <returns>Entity of T</returns>
        public static Task<IEnumerable<T>> GetMultipleAsync<T>(this IDbConnection connection, IEnumerable<object> idList, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            var type = typeof(T);
            ...
            {
                GetSingleKey<T>(nameof(GetMultiple));
                var name = GetTableName(type);
            ...
            foreach (var id in idList)
            {
                ...
                adapter.AppendColumnName(sqlCondition, key.Name);
                sqlCondition.Append($" = @id{index}");
                dynParms.Add($"@id{index++}", id);
            }
            if (sqlCondition.Length == 0)
            {
            ...
            return GetAllAsyncImpl<T>(connection, transaction, commandTimeout, sql, dynParms, type);
        }

        /// <summary>
        /// Returns a list of entites from table "Ts" matching specified condition.
        /// Id of T must be marked with [Key] attribute.
        /// Entities created from interfaces are tracked/intercepted for changes and used by the Update() extension
        /// for optimal performance.
        /// </summary>
        /// <typeparam name="T">Interface or type to create and populate</typeparam>
        /// <param name="connection">Open SqlConnection</param>
        /// <param name="sqlCondition">Sql condition for where statement.</param>
        /// <param name="paramters">Paramters for sql condition.</param>
        /// <param name="transaction">The transaction to run under, null (the default) if none</param>
        /// <param name="commandTimeout">Number of seconds before command execution timeout</param>
        /// <returns>Entity of T</returns>
        public static Task<IEnumerable<T>> GetAllByConditionAsync<T>(this IDbConnection connection, string sqlCondition, IDictionary<string, object> parameters, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            var type = typeof(T);
            var cacheType = typeof(List<T>);
            if (!GetQueries.TryGetValue(cacheType.TypeHandle, out string sql))
            ...
            {
                dynParms.Add(parameter.Key, parameter.Value);
            }
            sql = sql + " where " + sqlCondition;
            if (!type.IsInterface)
            {
        ...
        private static async Task<IEnumerable<T>> GetAllAsyncImpl<T>(IDbConnection connection, IDbTransaction transaction, int? commandTimeout, string sql, DynamicParameters dynParms,  Type type) where T : class
        {
            var result = await connection.QueryAsync(sql, dynParms, transaction: transaction, commandTimeout: commandTimeout).ConfigureAwait(false);
            var list = new List<T>();
