...
namespace vlko.core.DBAccess.SQLite
{
    public class SQLiteSession : ISession
    {
        ...
        internal SQLiteSession(SqliteConnection connection, Guid sessionIdent)
        {
        ...
        public SqliteTransaction NativeTransaction { get => _currentTransaction?.InnerTransaction; }
        ...
        public TCommand GetCommand<TCommand>() where TCommand : ICommands
        {
            ...
            if (command == null)
            {
                new SQLiteAccessException($"Command of type '{typeof(TCommand).Name}' was not resolved by current session scope!!");
            }
            if (!(command is ICommands<SQLiteSession>))
            {
                new SQLiteAccessException($"Command of type '{typeof(TCommand).Name}' does not implement ICommand<RavenSession> interface!!");
            }
            (command as ICommands<SQLiteSession>).Init(this);
            return command;
        }
        ...
        public ITransaction StartTransaction()
        {
            if (_currentTransaction != null)
            {
                ...
                throw new SQLiteAccessException($"There is already active transaction in this session [{SessionIdent}]!");
            }
            _currentTransaction = new SQLiteTransaction(this);
        ...
        private void DisposeResources(bool disposeManagedResources)
        {
            if (disposeManagedResources)
            {
                ...
                if (_connection != null)
                {
                    ...
                    SQLiteDatabase.Logger.Trace($"Session [{SessionIdent}] closed");
                    _connection.Dispose();
                }
            }
        }
