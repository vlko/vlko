	...
	public abstract class RssFeedActionBaseTest : LocalTest
	{
		...
		public virtual void Init()
		{
			...
			using (var tran = RepositoryFactory.StartTransaction())
			{
				...
				TestProvider.Create(feed1);
				TestProvider.Create(feed2);
				TestProvider.Create(feed3);
		...
		public virtual void Test_delete()
		{
			using (RepositoryFactory.StartUnitOfWork())
			{
				var initialCount = TestProvider.Count<RssFeed>();
				...
				Assert.AreEqual(initialCount + 1, TestProvider.Count<RssFeed>());
				...
				Assert.AreEqual(initialCount, TestProvider.Count<RssFeed>());
			}
		}
		...
		public virtual void Data_get_feed_to_process()
		{
			using (RepositoryFactory.StartUnitOfWork())
			{
				var dataActions = RepositoryFactory.Command<IRssFeedCommands>();
